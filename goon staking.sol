...
contract GOONGYE_NFT_STAKING is Ownable{
    ...
    function CheckStaked() public view returns(bool){
        return Road_Staking.isStaked(msg.sender);
    }

    function calcTime() public view returns(uint256){
        uint256 noOfDays = (block.timestamp - User[msg.sender].stakeTime).div(time);
        return noOfDays;
    }
    function perNFTReward() public view returns(uint256){
        bool check = CheckStaked();
    ...
    //     uint256 reward;
    //     uint256 noOfDays = calcTime();
    //     uint256 rewardPerNFT = perNFTReward();
    //     uint256[] memory arrNFT = userStakedNFT(msg.sender);
    //     // require(time >= noOfDays, "error");
    ...
    function Deduction() public view returns(uint256, uint256){
        uint256 Rew = userRewards();;
        uint256 userRew;
        userRew = Rew - (Rew/100*3);
        uint256 ownerRew;
        ownerRew = Rew - (Rew/100*97);
        return (userRew, ownerRew);
    }

    function WithdrawReward()  public {

    //    uint256 reward = rewardOfUser(msg.sender);
       (uint256 userReward, uint256 ownerRew) = Deduction();
       require(userReward > 0,"you don't have reward yet!");
    ...
    */
    ...
    // function rewardOfUser(address Add, uint256 _id) public view returns(uint256) {
    //     uint256 RewardToken;
